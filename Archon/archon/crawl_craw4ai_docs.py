    ...
    try:
        ...
        if not result.get('title') or not result.get('summary'):
            ...
            if not result.get('title'):
                result['title'] = f"Crawl4AI Documentation: {page_name}"
        ...
        first_line = chunk.split('\n')[0][:50].strip() if chunk else "Crawl4AI Documentation"
        title = first_line if first_line else f"Crawl4AI Documentation: {page_name}"
...
def clear_existing_records(supabase_client = None):
    ...
    try:
        ...
        result = client.table("site_pages").delete().eq("metadata->>source", "crawl4ai_docs").execute()
...
# Alias the async version to be compatible with older code
start_crawl_with_requests_async = start_crawl_with_requests

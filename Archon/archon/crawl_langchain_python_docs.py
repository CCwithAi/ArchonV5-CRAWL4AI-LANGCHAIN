...
async def get_title_and_summary(chunk: str, url: str) -> Dict[str, str]:
    ...
    try:
        ...
        # Validate that we have non-empty title and summary
        if not result.get('title') or not result.get('summary'):
            # Generate fallback title and summary
            path = urlparse(url).path
            page_name = path.split('/')[-1].replace('-', ' ').replace('_', ' ').title() or "Documentation"
            
            if not result.get('title'):
                result['title'] = f"Langchain Documentation: {page_name}"
            
            if not result.get('summary'):
                # Create a simple summary from the first 100 characters
                content_preview = chunk[:100].replace('\n', ' ').strip()
                result['summary'] = f"This documentation chunk contains information about {content_preview}..."
        
        return result
    except Exception as e:
        print(f"Error getting title and summary: {e}")
        # Generate fallback title and summary instead of returning error messages
        path = urlparse(url).path
        page_name = path.split('/')[-1].replace('-', ' ').replace('_', ' ').title() or "Documentation Page"
        
        # Get first line for title if possible
        first_line = chunk.split('\n')[0][:50].strip() if chunk else "Langchain Documentation"
        title = first_line if first_line else f"Langchain Documentation: {page_name}"
        
        # Create a simple summary from the first 100 characters
        content_preview = chunk[:100].replace('\n', ' ').strip() if chunk else "Documentation content"
        summary = f"This documentation chunk contains information about {content_preview}..."
...
async def main_with_requests(tracker: Optional[CrawlProgressTracker] = None):
    ...
    try:
        ...
        clear_existing_records()  # Remove the explicit passing of supabase client
        if tracker:
            tracker.log("Existing records cleared")
        else:
            print("Existing records cleared")
        
        # Get URLs from LangChain Python docs
        if tracker:
            tracker.log("Fetching URLs from LangChain Python sitemap...")
        else:
            print("Fetching URLs from LangChain Python sitemap...")
        urls = get_langchain_python_docs_urls()

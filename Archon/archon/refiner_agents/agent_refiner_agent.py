...
async def retrieve_relevant_documentation(ctx: RunContext[AgentRefinerDeps], query: str, sources: Optional[List[str]] = None) -> str:
    """
...
@agent_refiner_agent.tool
async def list_documentation_pages(ctx: RunContext[AgentRefinerDeps], source: Optional[str] = None) -> List[str]:
    """
...
@agent_refiner_agent.tool
async def get_page_content(ctx: RunContext[AgentRefinerDeps], url: str, source: Optional[str] = None) -> str:
    """
    Retrieve the full content of a specific documentation page by combining all its chunks.
    Use this tool to get pages related to setting up agents with Pydantic AI as well as using it for Crawl4AI and Langchain Python.
